...
test("assertApiAllowed accepts localhost wildcard ports", () => {
  assert.doesNotThrow(() => assertApiAllowed("http://127.0.0.1:3000/health", ["http://127.0.0.1:*"]));
  assert.doesNotThrow(() => assertApiAllowed("http://localhost:8080/health", ["http://localhost:*"]));
});
